...
<?php
	...
	$r = mysqli_query($sConn,"SELECT * FROM person  WHERE kategorija = 'Molba' AND idt='2' ORDER BY data_added DESC");
	...
	if($count>0)
	{
		while($row=mysqli_fetch_array($r))
		{
			...
			$izjavi.="<tr bgcolor=\"white\" style=\"border:1px solid black;\"><td><b>$idk$id_m</b></td><td><a href =\"lista_prikazi.php?id=".$row["id"]."\">$ime</a></td><td><b>$data_added</b></td></tr>";
		}
		$izjavi.="</table>";
	}
		...
		$izjavi="Нема внесено молба преку формулар";
	}
	...
	$re = mysqli_query($sConn,"SELECT * FROM person  WHERE kategorija = 'Molba' AND idt='1' ORDER BY data_added DESC");
	...
	if($countt>0)
	{
		while($row=mysqli_fetch_array($re))
		{
			...
			$data_added=strftime("%d/%m/%Y %I:%M", strtotime($row["data_added"]));
			$izjavii.="<tr bgcolor=\"white\" style=\"border:1px solid black;\"><td><b>$idk$id_m</b></td><td><a href ='upload/$NameFile'>$ime </a></td><td><b>$data_added</b></td></tr>";
			//$izjavii.="<br> <a href =\"izjava_prikacena.php?idf=".$row["idf"]."\">$data_added - $idf</a>";
		}
		$izjavii.="</table>";
	}
	else {
		$izjavii="Нема прикачено молба";
	}
?>
